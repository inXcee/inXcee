...
**Canlı commit:** `4bfe69b` (avskamp.com) — Shifts S1 + LaundryHub L1b + RoomsSection L1b + **Transport T1 TAM** deploy edildi (3 Haz 2026)
...
- **Transport** `TransportPage` 1872 → **148 satır ✓ — T1 TAMAM (saf orkestratör)** (3 Haz 2026): paylaşılan primitive'ler+helper `shared.jsx`'e (Section/BarList/Stat/Empty/ModalShell/Label/ModalActions/EmptyState/KPI + todayStr/toast/toastErr) (`373ec9d`); 5 sekme `tabs/`'e — ReportsTab (`8878cf4`), PointsTab+PointFormModal+MapPicker (`1ba69fc`), RoutesTab+RouteFormModal+StopsModal+RouteMap (`99a2f6b`), PeopleTab+StaffDetailDrawer+BulkImportModal (`9e4b737`), DailyTab+ManifestDrawer+ManualAssignButton+pdf/wa helper (`?`). TransportPage artık sekme yönlendirme + GlobalSearch. 13 smoke testi (frontend suite 144 yeşil), build OK. Davranış birebir korundu. **DEPLOY EDİLDİ `4bfe69b` (3 Haz 2026).** (Opsiyonel: GlobalSearch ayrı dosyaya — düşük öncelik.)
- **Capacity** `CapacityPage` 1587 → **294 satır ✓ — TAMAM (saf orkestratör)** (3 Haz 2026): sunum hücreleri+`roomCls` `shared.jsx`'e (RoomCell/GhostCell/FacilityCell); koridor düzeni `CorridorPlan.jsx`'e; oda detayı `RoomDetailPanel.jsx`'e (RoomDetailPanel+ZimmetPanel+UnreturnedZimmetModal); odasız personel havuzu `UnassignedPool.jsx`'e (`6bc3617`). CapacityPage artık blok/kat/filtre kontrolü + plan/detay/havuz yönlendirmesi. 7 smoke testi, build OK, davranış birebir. **DEPLOY EDİLDİ `55f45d0` (3 Haz 2026).** (Opsiyonel düşük: RoomDetailPanel 807 satır iç bölme.)
- **Maintenance** `MaintenancePage` 1386 → **376 satır ✓ — M1b TAMAM (saf orkestratör)** (3 Haz 2026): sabitler/helper'lar + SLACountdown/StatusTimeline/StatusActions/PhotoCapture `shared.jsx`'e; konum seçici `LocationPicker.jsx`'e; sürükle-bırak panosu `KanbanView.jsx`'e; teknisyen panelleri `TechnicianPanels.jsx`'e; arıza detayı `DetailPanel.jsx`'e (`bfe0eac`). MaintenancePage artık KPI/filtre/liste-kanban + form/detay yönlendirmesi. 8 smoke testi, build OK, davranış birebir. **DEPLOY EDİLDİ `55f45d0` (3 Haz 2026).**
- **CheckinPage** 1033 → **554 satır ✓ — C1b TAMAM (sihirbaz orkestratör)** (3 Haz 2026): adım çubuğu+autocomplete `shared.jsx`'e (StepBar/AutoInput/STEPS/INIT_FORM_DATA); oda seçici `RoomPicker.jsx`'e; istatistik panosu `StatsDashboard.jsx`'e; hızlı doluluk modalı `QuickFillModal.jsx`'e (prop-alan) (`?`). Sihirbazın 4 adımı orkestratörde kaldı (~30 paylaşılan state — ayırmak ağır prop-drilling). 6 smoke testi, build OK, davranış birebir. **DEPLOY EDİLMEDİ.**
- **KALAN decomposition:** **DisciplinePage** ~926 · HousekeepingPage 1520 · CampusMapPage 2144
...
- **Cards:** ✅ **Telefonla kayıt (Android NFC + foto) TAMAM** (migration 009 cards.photo_url; POST /cards/:id/photo upload; ortak `shared/nfc.js` normalizeNfcUid bind-nfc+issue+stations/scan'de → telefon ↔ istasyon UID eşleşir; CardsPage 📱 NFC OKU (Web NFC NDEFReader, feature-detect) + 📷 Foto çek/değiştir + thumbnail; 9 backend + 2 frontend test). iPhone NFC/OCR/QR kapsam dışı. — **DEPLOY EDİLDİ (`cd8aefd`)** · ✅ **Toplu basım TAMAM** (`GET /cards/batch-pdf?card_type=&ids=`, A4'te 2×4=8 kart/sayfa, drawCard saf helper — tek-kart endpoint de kullanır; CardsPage "⊞ toplu PDF" butonları; 5 backend + 1 frontend test) — **DEPLOY EDİLDİ (`0eba714`)** · ✅ **Kart analitiği TAMAM** (`cards/analytics.js`: summary[kapsam/NFC-bağlı/kayıp] + usageByDay + usageByResult + topStations access_events'ten; `GET /cards/analytics`; CardsPage 📊 Analiz toggle görünümü; 8 backend + 1 frontend test) — **DEPLOY EDİLMEDİ**. **Cards spec kalemlerinin TÜMÜ bitti.**
