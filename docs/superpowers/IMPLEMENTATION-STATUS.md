...
- **Laundry** (L1b): `LaundryHub` 2038 → **825 satır ✓** — iç bileşenler `components/`'e: QuickNotes+DeliveredTodaySection (`6f43290`), Kanban kümesi→`KanbanBoard.jsx` (ExpandedSection/DraggableKanbanCard/KanbanCard/KanbanCol + COLOR_MAP/GARMENT_COLOR_HEX/waLink) (`a84a3bd`), QuickAdd+FullRecordsView (`0ed7562`). 4 smoke testi (suite 112 yeşil). LaundryHub artık kanban board orkestrasyonu + records/reports/settings yönlendirmesi. `components/RoomsSection.jsx` 2026 → **1647 satır (DEVAM EDİYOR)**: analiz/grafik bileşenleri `components/roomsAnalytics.jsx`'e (Stat/Sparkline/TlChipGroup/YearHeatmap/HourDayMatrix/BlockCompareCard/SlaCard/ItemActionMenu/DamagesCard) + 5 smoke (`67f3c5c`). **KALAN:** önce paylaşılan sabit/helper'lar (`STATUS_LABEL`/`STATUS_COLOR`/`formatRelative` — RoomCard+PremiumGarmentsCard+RoomDetailPanel ortak kullanıyor) bir `roomsShared.js`'e çıkarılmalı, sonra RoomCard/OccupantRow/PremiumGarmentsCard, en sonda iki dev: **RoomDetailPanel (~585 satır)** + **InlineNewRecord (~405 satır)**. Bu son adımlar iç içe geçmiş — taze bağlamla yapılması önerilir.
