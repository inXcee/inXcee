...
- **Cards:** ✅ **Telefonla kayıt (Android NFC + foto) TAMAM** (migration 009 cards.photo_url; POST /cards/:id/photo upload; ortak `shared/nfc.js` normalizeNfcUid bind-nfc+issue+stations/scan'de → telefon ↔ istasyon UID eşleşir; CardsPage 📱 NFC OKU (Web NFC NDEFReader, feature-detect) + 📷 Foto çek/değiştir + thumbnail; 9 backend + 2 frontend test). iPhone NFC/OCR/QR kapsam dışı. — **DEPLOY EDİLDİ (`cd8aefd`)** · ✅ **Toplu basım TAMAM** (`GET /cards/batch-pdf?card_type=&ids=`, A4'te 2×4=8 kart/sayfa, drawCard saf helper — tek-kart endpoint de kullanır; CardsPage "⊞ toplu PDF" butonları; 5 backend + 1 frontend test) — **DEPLOY EDİLMEDİ** · kart analitiği (kalan)
