...
function DeptBadge({ name, colorClass }) {
  ...
  return <span className={`px-2 py-0.5 rounded text-xs ${bgMap[colorClass] || 'bg-slate-700 text-slate-300'}`}>{name}</span>
}
...
function ScheduleTab({ departments, shiftDefs }) {
  ...
  const [editModal, setEditModal] = useState(null) // { personnelId, date, deptId }
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <button onClick={() => setWeekStart(addDays(weekStart, -7))} className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 rounded text-sm text-slate-300">← Önceki</button>
        <span className="text-sm text-slate-300 font-medium">
          {formatDate(weekStart)} – {formatDate(weekEnd)}
        </span>
        <button onClick={() => setWeekStart(addDays(weekStart, 7))} className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 rounded text-sm text-slate-300">Sonraki →</button>
        <button onClick={() => setWeekStart(getWeekStart(new Date()))} className="px-3 py-1.5 bg-slate-700 hover:bg-slate-600 rounded text-sm text-slate-300">Bu Hafta</button>
        <select
          value={deptFilter}
      ...
      <div className="flex gap-4 mb-4 flex-wrap text-xs text-slate-400">
        {shiftDefs.map(s => {
          const bgMap = { 'bg-blue-400': 'bg-blue-400', 'bg-orange-400': 'bg-orange-400', 'bg-indigo-600': 'bg-indigo-600' }
          return (
            <span key={s.id} className="flex items-center gap-1">
              <span className={`w-3 h-3 rounded ${bgMap[s.color_class] || 'bg-slate-500'}`}></span>
      ...
      {isLoading ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs border-collapse">
            <thead>
              <tr>
                <th className="text-left p-2 bg-slate-900 text-slate-400 font-medium sticky left-0 min-w-40">Personel</th>
                <th className="text-left p-2 bg-slate-900 text-slate-400 font-medium min-w-24">Bölüm</th>
                {weekDays.map(d => (
                  <th key={d} className="text-center p-2 bg-slate-900 text-slate-400 font-medium min-w-28">
                ...
                <tr key={person.id} className="border-t border-slate-800 hover:bg-slate-900/50">
                  <td className="p-2 sticky left-0 bg-slate-950">
                    <div className={`flex items-center gap-1 border-l-4 pl-2 ${person.gender === 'female' ? 'border-pink-400' : 'border-blue-400'}`}>
                      ...
                      <span className="text-slate-200 truncate max-w-32">{person.full_name}</span>
                    </div>
                  </td>
                  <td className="p-2">
                        ...
                        {cell ? (
                          ...
                          >
                            {cell.status === 'on_leave' ? (
                              <span className="block px-1 py-0.5 rounded bg-amber-500/20 text-amber-300 text-xs">İzin</span>
                            ) : cell.status === 'absent' ? (
      ...
      {editModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setEditModal(null)}>
          <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 w-72" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-2 mb-4">
              {shiftDefs.map(s => (
                <button
                  ...
                  className={`w-full px-3 py-2 rounded border-2 text-sm text-left transition-colors ${editShiftDef === s.id.toString() ? 'border-blue-500 bg-blue-900/20 text-blue-200' : 'border-slate-700 text-slate-400 hover:border-slate-600'}`}
                >
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 text-white rounded text-sm"
              >
                {updateShift.isPending ? 'Kaydediliyor...' : 'Kaydet'}
              </button>
              <button onClick={() => setEditModal(null)} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded text-sm">İptal</button>
            </div>
          </div>
        </div>
      )}
...
function LeaveTab({ departments }) {
  ...
  const statusBadge = s => {
    ...
    return <span className={`px-2 py-0.5 rounded text-xs ${m[s]}`}>{l[s]}</span>
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <select value={filters.leave_type} onChange={e => setFilters(p => ({ ...p, leave_type: e.target.value }))}
          className="bg-slate-800 border border-slate-700 rounded px-3 py-1.5 text-sm text-slate-300">
          <option value="">Tüm İzin Türleri</option>
          {Object.entries(LEAVE_TYPES).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
        </select>
        <button onClick={() => setNewLeave(true)} className="ml-auto px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded text-sm">+ Yeni İzin</button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        {Object.entries(LEAVE_TYPES).map(([k, v]) => {
          ...
          return (
            <div key={k} className={`rounded-lg p-3 ${v.color} border border-current/20`}>
              <div className="text-lg font-bold">{count}</div>
              <div className="text-xs opacity-80">{v.label} İzni</div>
            </div>
          )
        })}
      </div>
          ...
          <tbody>
            {leaves.map(l => (
              <tr key={l.id} className="border-t border-slate-800 hover:bg-slate-800/50">
                ...
                <td className="p-3 text-slate-400 text-xs">{formatDate(l.start_date)} – {formatDate(l.end_date)}</td>
      ...
      {/* New Leave Modal */}
      {newLeave && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setNewLeave(false)}>
          <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 w-96" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={form.personnel_id} onChange={e => setForm(p => ({ ...p, personnel_id: e.target.value }))}
                  placeholder="Personel ID" className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm text-slate-100" />
              </div>
              <div>
                ...
                <select value={form.leave_type} onChange={e => setForm(p => ({ ...p, leave_type: e.target.value }))}
                  className="w-full bg-slate-800 border border-slate-700 rounded px-3 py-2 text-sm text-slate-300">
                  {Object.entries(LEAVE_TYPES).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
                </select>
              </div>
            ...
            <div className="flex gap-2 mt-4">
              ...
              <button onClick={() => setNewLeave(false)} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded text-sm">İptal</button>
            </div>
          </div>
        </div>
      )}
...
function OvertimeTab({ departments }) {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['overtime'] }); qc.invalidateQueries({ queryKey: ['overtime-summary'] }); setShowForm(false); setForm({ personnel_id: '', work_date: '', hours: '', reason: '' }) },
    onError: e => alert(e.response?.data?.error || 'Hata'),
  })
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        ...
        {canAdd && (
          <button onClick={() => setShowForm(true)} className="ml-auto px-4 py-1.5 bg-purple-700 hover:bg-purple-600 text-white rounded text-sm">+ Mesai Ekle</button>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        ...
        <StatCard label="Ort. Saat/Kişi" value={records.length ? (totalHours / new Set(records.map(r => r.personnel_id)).size).toFixed(1) + 's' : '—'} color="text-orange-400" />
      </div>
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 w-96" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={() => createMut.mutate({ ...form, personnel_id: parseInt(form.personnel_id), hours: parseFloat(form.hours) })}
                ...
                className="flex-1 px-4 py-2 bg-purple-700 hover:bg-purple-600 disabled:bg-slate-700 text-white rounded text-sm">
                {createMut.isPending ? 'Kaydediliyor...' : 'Kaydet'}
              </button>
              <button onClick={() => setShowForm(false)} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded text-sm">İptal</button>
            </div>
          </div>
        </div>
      )}
...
// ─── Tab: İstatistikler ─────────────────────────────────────────────────────
function StatisticsTab({ departments }) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 rounded-xl p-4">
        ...
        <div className="flex items-center gap-6">
          <div className="text-center">
            <div className="text-3xl font-bold text-blue-400">{totalMale}</div>
            <div className="text-xs text-slate-400 mt-1">♂ Erkek</div>
          </div>
          ...
          <div className="text-center">
            <div className="text-3xl font-bold text-pink-400">{totalFemale}</div>
            <div className="text-xs text-slate-400 mt-1">♀ Kadın</div>
          </div>
        </div>
      </div>
      ...
      {stats.byShift?.length > 0 && (
        <div className="bg-slate-900 rounded-xl p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {stats.byShift.map(s => {
              const bgMap = { 'bg-blue-400': 'border-blue-400', 'bg-orange-400': 'border-orange-400', 'bg-indigo-600': 'border-indigo-500' }
              return (
                <div key={s.shift_def_id} className={`bg-slate-800 rounded-lg p-4 border-l-4 ${bgMap[s.color_class] || 'border-slate-600'}`}>
      ...
      <div className="bg-slate-900 rounded-xl p-4">
        ...
        <div className="space-y-2">
          {deptSummary.map(d => {
            const borderMap = {
              'bg-red-600': 'border-red-600', 'bg-green-600': 'border-green-600',
              'bg-orange-500': 'border-orange-500', 'bg-blue-600': 'border-blue-600',
              'bg-yellow-500': 'border-yellow-500', 'bg-lime-500': 'border-lime-500',
              'bg-pink-500': 'border-pink-500', 'bg-purple-600': 'border-purple-600',
            }
            const maxCount = Math.max(...deptSummary.map(x => x.personnel_count), 1)
            return (
              <div key={d.id} className="flex items-center gap-3">
                <div className="w-24 text-xs text-slate-400 text-right truncate">{d.name}</div>
      ...
      {/* Overtime summary */}
      <div className="bg-slate-900 rounded-xl p-4">
        <h3 className="font-medium text-slate-200 mb-1">Bu Ay Mesai</h3>
...
export default function ShiftsPage() {
  ...
  return (
    <div className="max-w-full">
      <h1 className="text-xl font-bold text-slate-100 mb-2">Vardiya Yönetimi</h1>
      <p className="text-sm text-slate-400 mb-6">200 personel · 3 vardiya · 8 bölüm</p>
      ...
      <div className="flex gap-1 mb-6 border-b border-slate-800">
        {tabs.map(t => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeTab === t.id ? 'border-blue-500 text-blue-400' : 'border-transparent text-slate-400 hover:text-slate-300'}`}
          >
            {t.label}
          </button>
        ))}
      ...
      {activeTab === 'statistics' && <StatisticsTab  departments={departments} />}
    </div>
  )
}
